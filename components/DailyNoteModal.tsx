...
interface DailyNoteData {
  ...
  etf: {
    rows: any[];  // each row: { asset, flow, aum, flowSource, aumSource }
    strategyValue: number | null;
    strategyHoldings: number | null;
    strategyAvgPrice: number | null;
    bitmineValueUsd: number | null;
    bitmineHoldings: number | null;
    bitmineAvgPrice: number | null;
  };
}
...
function buildHTML(note: DailyNoteData): string {
  ...
  // Price formatter: adapts decimals to magnitude (BTC ~$62k vs XRP ~$3)
  ...
  const spotRowsHTML = spot.rows.map(r => `
    <tr>
      ...
      <td style="padding:4px 10px;font-size:12px;border-bottom:1px solid #e5e7eb;text-align:right;font-weight:600">${fmtPrice(r.price)}</td>
  ...
  const fmtAum = (v: number | null | undefined, dash?: string) =>
    v == null ? '<span style="color:#9ca3af;font-style:italic">Data Not Published</span>' : fmtUSD(v);

  // ETF rows: XRP flow shows dash with tooltip if null (Farside doesn't track XRP),
  // otherwise shows CoinGlass figure
  const etfRowsHTML = etf.rows.map(r => {
    let flowCell: string;
    if (r.flow != null) {
      flowCell = `<span style="color:${flowColor(r.flow)}">${fmtFlow(r.flow)}</span>`;
    } else if (r.asset === 'XRP') {
      // XRP flow: dash with note that it's not tracked by Farside
      // If we ever get it from CoinGlass it will show above; null means CoinGlass also failed
      flowCell = '<span style="color:#9ca3af" title="Not tracked by Farside Investors">—</span>';
    } else {
      flowCell = '—';
    }
    return `
    <tr>
      <td style="padding:4px 10px;font-size:12px;border-bottom:1px solid #e5e7eb;font-weight:600">${r.asset}</td>
      <td style="padding:4px 10px;font-size:12px;border-bottom:1px solid #e5e7eb;text-align:right">${flowCell}</td>
...
<div style="font-family:Arial,sans-serif;max-width:600px;color:#1a1917">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:4px">
    <tr>
      ...
      <th style="padding:5px 10px;font-size:11px;font-weight:700;background:#1a1917;color:#fff;text-align:right">Price</th>
  ...
  <div style="font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#1a1917;padding:0 10px 4px">Funding &amp; Liquidations</div>
  <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:10px">
    <tr>
      <th style="padding:5px 10px;font-size:11px;font-weight:700;background:#1a1917;color:#fff;text-align:left"></th>
      <th style="padding:5px 10px;font-size:11px;font-weight:700;background:#1a1917;color:#fff;text-align:right">Latest Funding Rate</th>
      <th style="padding:5px 10px;font-size:11px;font-weight:700;background:#1a1917;color:#fff;text-align:right">7D Funding Rate</th>
    </tr>
    ${fundingRowsHTML}
  </table>
  <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:16px">
  ...
  <div style="font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#1a1917;padding:8px 10px 4px">BitMine (BMNR)</div>
  <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:16px">
    <tr>
      <td style="padding:4px 10px;font-size:12px;border-bottom:1px solid #e5e7eb;color:#6b7280">Value of Holdings</td>
